...
while robot.step(timestep) != -1:
    ...
    if (us[0].getValue() > 200) and (us[1].getValue() > 200): #obstaculo de frente
        posl = posl + .25
        posr = posr + .25
        print("atras")
    ...
    elif (us[0].getValue() > 200) and (us[1].getValue() > 200): #obstaculo a la izquierda
        posl = posl - .25
        posr = posr + .25
        print("derecha")
    elif (us[0].getValue() > 200) and (us[1].getValue() > 200): #obstaculo a la derecha
        posl = posl + .25
        posr = posr - .25
        print("izquierda")
